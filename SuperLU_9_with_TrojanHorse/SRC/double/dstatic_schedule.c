...
int dstatic_schedule(superlu_dist_options_t *options, int m, int n,
                     ...
                     int_t *perm_c_supno, int_t *iperm_c_supno, int *info)
{
    ...
    *info = 0;
          ...
          options->RowPerm == NOROWPERM))) /* no rowperm to destroy symmetry */
    {
        ...
        {   /* ParSymbFACT==YES and SymPattern==YES and RowPerm == NOROWPERM */
        ...
        for (i = 0; i < nsupers; i++)
            num_child[i] = 0;
        for (i = 0; i < nsupers; i++)
            if (etree_supno[i] != nsupers)
                num_child[etree_supno[i]]++;
    ...
    { /* Unsymmetric pattern */
        ...
        if (!(blockr = intMalloc_dist(nblocks)))
            ABORT("Malloc fails for blockr[].");
        log_memory(nblocks * iword, stat);
        ...
        nlb = nsupers / Pc;
        if (!(Ublock = intMalloc_dist(nblocks)))
            ABORT("Malloc fails for Ublock[].");
        if (!(Urows = intMalloc_dist(1 + nlb)))
            ABORT("Malloc fails for Urows[].");
        ...
        if (!(Lblock = intMalloc_dist(nblocks)))
            ABORT("Malloc fails for Lblock[].");
        if (!(Lrows = intMalloc_dist(1 + ncb)))
            ABORT("Malloc fails for Lrows[].");
    ...
    for (lb = 0; lb < nsupers; lb++)
        iperm_c_supno[perm_c_supno[lb]] = lb;
