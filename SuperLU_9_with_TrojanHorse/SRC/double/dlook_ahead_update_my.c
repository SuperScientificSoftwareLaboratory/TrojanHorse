...
#endif
{
    ...
    {
        segsize = klst - usub[jj];
        printf("usub[%d]=%d segsize=%d rukp=%d\n", jj, usub[jj], segsize, rukp);
        if (segsize)
        {
    ...
    {                   /* Loop through each block in L(:,k) */
       ...
       different lb.   */
        ...
        printf("SSSSM Golden.L.before\n");
        for(int j=0;j<ldu;j++){
            for(int i=0;i<temp_nbrow;i++){
                int lyd_idx = j * nsupr + i;
                printf("(%d,%d) offset=%d %le\n", i,j,lyd_idx,lusup[luptr + (knsupc - ldu) * nsupr + lyd_idx]);
            }
        }
        // printf("SSSSM Golden.U.before\n");
        ...
        printf("dgemm_ %d %d %d  %d %d %d\n", temp_nbrow, ncols, ldu, nsupr, ldu, temp_nbrow);
