...
void dCreate_CompCol_Matrix_dist(SuperMatrix *A, int_t m, int_t n, int_t nnz,
                                 ...
                                 Stype_t stype, Dtype_t dtype, Mtype_t mtype)
{
    ...
    if (!(A->Store))
        ABORT("SUPERLU_MALLOC fails for A->Store");
...
void dCreate_CompRowLoc_Matrix_dist(SuperMatrix *A, int_t m, int_t n,
 ...
 */
void dCompRow_to_CompCol_dist(int_t m, int_t n, int_t nnz,
 ...
 */
void dCompCol_to_CompRow_dist(int_t m, int_t n, int_t nnz,
...
void dCreate_Dense_Matrix_dist(SuperMatrix *X, int_t m, int_t n, double *x,
                               ...
                               Mtype_t mtype)
{
    ...
    if (!(X->Store))
        ABORT("SUPERLU_MALLOC fails for X->Store");
    Xstore = (DNformat *)X->Store;
    Xstore->lda = ldx;
    Xstore->nzval = (double *)x;
}

void dCopy_Dense_Matrix_dist(int_t M, int_t N, double *X, int_t ldx,
                             double *Y, int_t ldy)
{
    /*! \brief
     *
...
void dCreate_SuperNode_Matrix_dist(SuperMatrix *L, int_t m, int_t n, int_t nnz,
                                   ...
                                   Stype_t stype, Dtype_t dtype, Mtype_t mtype)
{
    ...
    if (!(L->Store))
        ABORT("SUPERLU_MALLOC fails for L->Store");
    Lstore = L->Store;
    Lstore->nnz = nnz;
...
void dClone_CompRowLoc_Matrix_dist(SuperMatrix *A, SuperMatrix *B)
{
    ...
    if (!(B->Store))
        ABORT("SUPERLU_MALLOC fails for B->Store");
...
void dScaleAddId_CompRowLoc_Matrix_dist(SuperMatrix *A, double c)
{
    ...
    { /* Loop through each row */
        ...
        {
            if ((Astore->fst_row + i) == Astore->colind[j])
            { /* diagonal */
                temp = aval[j] * c;
                aval[j] = temp + one;
            }
 ...
 */
void dFillRHS_dist(char *trans, int_t nrhs, double *x, int_t ldx,
...
int dprint_gsmv_comm(FILE *fp, int_t m_loc, pdgsmv_comm_t *gsmv_comm,
                     gridinfo_t *grid)
{
...
void dGenXtrueRHS(int nrhs, SuperMatrix *A, Glu_persist_t *Glu_persist,
