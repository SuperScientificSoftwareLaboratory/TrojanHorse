...
int_t pdgstrs_init_device_lsum_x(superlu_dist_options_t *options, int_t n, int_t m_loc, int_t nrhs, gridinfo_t *grid,
...
/* nvshmem related. */
    ...
    tmp_val[0] = 0;
    for (int i = 1; i < h_nfrecvmod[3] + 1; i++)
        tmp_val[i] = -1;
    ...
    if (!(tmp_val_u = (int *)SUPERLU_MALLOC((h_nfrecvmod_u[3] + 1) * sizeof(int))))
        ABORT("Malloc fails for tmp_val[].");
    tmp_val_u[0] = 0;
    for (int i = 1; i < h_nfrecvmod_u[3] + 1; i++)
        tmp_val_u[i] = -1;
