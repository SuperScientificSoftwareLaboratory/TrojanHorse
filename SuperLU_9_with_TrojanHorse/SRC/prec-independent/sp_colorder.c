 ...
 * </pre>
 */
void sp_colorder(superlu_dist_options_t *options, SuperMatrix *A, int_t *perm_c,
				 int_t *etree, SuperMatrix *AC)
{
	...
	if (!ACstore)
		ABORT("SUPERLU_MALLOC fails for ACstore");
	...
	if (!(ACstore->colbeg))
		ABORT("SUPERLU_MALLOC fails for ACstore->colbeg");
	ACstore->colend = (int_t *)SUPERLU_MALLOC(n * sizeof(int_t));
	if (!(ACstore->colend))
		ABORT("SUPERLU_MALLOC fails for ACstore->colend");
	...
	if (options->Fact == DOFACT || options->Fact == SamePattern)
	{
		...
		if (!iwork)
			ABORT("SUPERLU_MALLOC fails for iwork[]");
		...
		// printf("Before postreorder etree: \n");
		// for(int i=0;i<n;i++){
		// 	printf("%d ", etree[i]);
		// }
		// printf("\n");
		...
		for (i = 0; i < n; ++i)
			iwork[post[i]] = post[etree[i]];
		for (i = 0; i < n; ++i)
			etree[i] = iwork[i];
		
		// printf("After postreorder etree: \n");
		// for(int i=0;i<n;i++){
		// 	printf("%d ", etree[i]);
		// }
		// printf("\n");
...
#if (DEBUGlevel >= 3)
		if (!iam)
			PrintInt10("postorder etree:", n, etree);
#endif

		/* Postmultiply A*Pc by post[] */
		for (i = 0; i < n; ++i)
			iwork[post[i]] = ACstore->colbeg[i];
		for (i = 0; i < n; ++i)
			ACstore->colbeg[i] = iwork[i];
		for (i = 0; i < n; ++i)
			iwork[post[i]] = ACstore->colend[i];
		for (i = 0; i < n; ++i)
			ACstore->colend[i] = iwork[i];
