...
void read_command_params(int argc, char **argv, char *mtx_name, char *rhs_name, int *nb)
{
    ...
    while ((c = getopt(argc, argv, "nb:f:r:")) != EOF)
    {
        ...
        case 'f':
            ...
            char file_path[120];
            sprintf(file_path, "ppopp_output/28_%d_%s.csv", nproc, mtx_name_glo);
            file_28 = fopen(file_path, "w");
            fprintf(file_28, "mtx_name, numeric_time\n");
            fflush(file_28);
...
int main(int ARGC, char **ARGV)
{
    ...
    init_options.nb = nb;
    init_options.gpu_kernel_warp_per_block = 2;
